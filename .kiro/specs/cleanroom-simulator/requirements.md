...
```
p_avg = Σ (m_i * p_i) / Σ (m_i)           # 流体セルの空間平均（m_i=mask）
p_i = m_i * (p_i - p_avg) + (1-m_i)*p_avg  # 流体セルはゼロ平均、物体セルはp_avg
```

※ 物体セル（mask=0）は平均値計算から除外され、引き戻し後はp_avgに設定する（外れ値抑制）
...
6. The Solver shall 対流流出条件の輸送速度を該当境界面の平均速度とする（擬似速度への境界適用時は **nステップ速度場** から算出）
...
※ `Outflow`の対流流出条件は対流項として与える。粘性項は特別扱いせず、マスクとゴーストセル値により自然に計算する。
...
各速度成分φに対して以下の式を適用する（時間離散は **n時刻の値** で評価）：
...
※ 壁面（粘着条件）は速度ゼロ
※ 擬似速度ベクトル（u*）に対して対流流出条件を適用する場合、輸送速度 Uc は **nステップ速度場（u^n）** から算出し、更新式の右辺も **u^n** を用いる（u* 自身からは算出しない）。
※ `Opening` の `outlet` でも同様に、領域平均速度は **u^n** を用いて算出する。
※ 流出境界の擬似速度セルフェイスは保存性を優先し、セルセンターとセルフェイス値から構成する。  
　例（x_max 面, 法線方向速度成分）  
　u*_{i+1/2} = u^n_{i+1/2} - 2 Δt Uc (u^n_{i+1/2} - u^n_i) / Δx  
　（u^n_{i+1/2} は前時刻のセルフェイス速度を保持する状態量）
※ 非等間隔格子では、セル中心〜フェイス間距離（Δx/2 など）の局所値を用いる。
※ ポアソン右辺（∇·u*）はセルフェイス速度を用いて評価する。
※ 速度補正後、流出境界のセルフェイス速度は連続の式から決定する。  
　例（x_max 面）  
　u^{n+1}_{i+1/2} = u^{n+1}_{i-1/2} - Δx \{ (v^{n+1}_{j+1/2}-v^{n+1}_{j-1/2})/Δy + (w^{n+1}_{k+1/2}-w^{n+1}_{k-1/2})/Δz \}  
　（x_min, y±, z± も同様に、境界隣接セルの連続式から法線成分を算出）
※ Opening の outlet も同様に、流出境界としてフェイス更新・連続式補正の対象とする。
※ 逆流時の安定化（reverse_flow_stabilization）を有効にする場合は、Uc<0 の寄与をゼロクリップして更新する。
※ Outflowのセル中心境界条件（ゴーストセル値）は、WENO等の対流項計算に必要なため引き続き適用する。

**具体式（例: x_max 面の法線方向速度）**

セルフェイス擬似速度（保存的更新）:

```
u_face_n = u_face_x[i+1/2]^n
u_face_x[i+1/2]^* = u_face_n - 2 * Δt * Uc * (u_face_n - u_i^n) / Δx
```

- `u_face_x[i+1/2]^n` は前時刻に保持されたセルフェイス速度
- `i` は境界隣接の内点（x_max面では i = mx-2）
- `Δx` は局所セル幅（非等間隔格子では中心-フェイス距離を用いる）
...
u_face_x[i+1/2]^{n+1} =
  u_face_x[i-1/2]^{n+1}
  - Δx * ( (v_face_y[i, j+1/2]^{n+1} - v_face_y[i, j-1/2]^{n+1}) / Δy
          + (w_face_z[i, j, k+1/2]^{n+1} - w_face_z[i, j, k-1/2]^{n+1}) / Δz )
```

（x_min, y±, z± も同様に、境界隣接セルの連続式で法線成分を算出）
...
8. The Solver shall 物体内部の圧力は平均値引き戻し後に平均値で埋める（可視化時の外れ値抑制）
...
```json
{
  ...
  "debug": "no",                          // (yes | no) デバッグ診断出力（安定条件違反時の詳細ログ）
...
※ 外部境界に Inflow/Outflow が存在する場合、history間隔ごとに外部境界面の無次元流量を履歴ファイルへ出力する。  
  - 出力ヘッダ: `step, time, x-, x+, y-, y+, z-, z+, sum`  
  - 外向き正の正味流量（各面）と全6面の合計（sum）
※ `debug` が有効な場合は、display間隔ごとに Inflow/Outflow 境界面の流量モニタ（out/in分離、un最小/最大）を標準出力に出力する。
※ 初期条件として、セルフェイス速度をセルセンター値から内挿して初期化する。
...
| 条件 | 式 | 用途 |
| ---- | -- | ---- |
| CFL条件（対流） | Co = max(Cx, Cy, Cz), Cx = |u|·Δt/Δx, Cy = |v|·Δt/Δy, Cz = |w|·Δt/Δz | Δt決定 |
...
{
  "external_boundaries": {
    "x_min": { "velocity": { "type": "Inflow", "value": [0.3, 0.0, 0.0] } },
    "x_max": { "velocity": { "type": "Outflow" } },
    "y_min": { "velocity": { "type": "Periodic" } },
    "y_max": { "velocity": { "type": "Periodic" } },
    "z_min": { "velocity": { "type": "Wall" } },
    "z_max": { "velocity": { "type": "Wall" } }
  },
  "openings": [
    {
...
※ 外部境界は `{ "velocity": { "type": "<bc_type>", "value": [...] } }` 形式のみを許可し、文字列のみの省略形は許可しない。
