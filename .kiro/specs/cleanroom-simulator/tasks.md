...
- [ ] 2.1: internal_boundaries読み込み時にGeometryと連携してmask=0を設定
- [ ] 2.2: 物体内部速度としてvelocityを適用する処理を実装
- [ ] 2.3: 境界条件優先順位の適用（Opening > internal_boundaries > 外部境界）
...
- [ ] 3.1: apply_pressure_bcs!をNeumann/Periodic分離で再実装
- [ ] 3.2: Outflow境界の圧力をDirichletからNeumannに変更
- [ ] 3.3: 圧力平均引き戻しのコメント・処理をPeriodic例外対応に修正
...
## Task 5: Outflow境界の粘性項ゼロ対応
**Requirements**: 4.1-4.9, 8.1-8.8

Outflow境界では粘性項をゼロとし、対流項のみで流出条件を与える

- [ ] 5.1: Diffusionモジュールで境界条件情報を受け取る引数追加
- [ ] 5.2: Outflow境界隣接セルの粘性項フラックスをゼロに設定
