...
function add_diffusion_flux!(
    ...
    @inbounds for k in 3:mz-2, j in 3:my-2, i in 3:mx-2
        ...
        m_c = buffers.mask[i, j, k]
        grad_w = (u_c - buffers.u[i-1, j, k]) / dx * buffers.mask[i-1, j, k] * m_c
        grad_e = (buffers.u[i+1, j, k] - u_c) / dx * buffers.mask[i+1, j, k] * m_c
        ...
        grad_s = (u_c - buffers.u[i, j-1, k]) / dy * buffers.mask[i, j-1, k] * m_c
        grad_n = (buffers.u[i, j+1, k] - u_c) / dy * buffers.mask[i, j+1, k] * m_c
        ...
        grad_b = (u_c - buffers.u[i, j, k-1]) / dz_m * buffers.mask[i, j, k-1] * m_c
        grad_t = (buffers.u[i, j, k+1] - u_c) / dz_p * buffers.mask[i, j, k+1] * m_c
        ...
        # --- v-component ---
        # x
        grad_w = (v_c - buffers.v[i-1, j, k]) / dx * buffers.mask[i-1, j, k] * m_c
        grad_e = (buffers.v[i+1, j, k] - v_c) / dx * buffers.mask[i+1, j, k] * m_c
        diff_v_x = (nu_e * grad_e - nu_w * grad_w) / dx
        
        # y
        grad_s = (v_c - buffers.v[i, j-1, k]) / dy * buffers.mask[i, j-1, k] * m_c
        grad_n = (buffers.v[i, j+1, k] - v_c) / dy * buffers.mask[i, j+1, k] * m_c
        diff_v_y = (nu_n * grad_n - nu_s * grad_s) / dy
        
        # z
        grad_b = (v_c - buffers.v[i, j, k-1]) / dz_m * buffers.mask[i, j, k-1] * m_c
        grad_t = (buffers.v[i, j, k+1] - v_c) / dz_p * buffers.mask[i, j, k+1] * m_c
        ...
        # --- w-component ---
        # x
        grad_w = (w_c - buffers.w[i-1, j, k]) / dx * buffers.mask[i-1, j, k] * m_c
        grad_e = (buffers.w[i+1, j, k] - w_c) / dx * buffers.mask[i+1, j, k] * m_c
        diff_w_x = (nu_e * grad_e - nu_w * grad_w) / dx
        
        # y
        grad_s = (w_c - buffers.w[i, j-1, k]) / dy * buffers.mask[i, j-1, k] * m_c
        grad_n = (buffers.w[i, j+1, k] - w_c) / dy * buffers.mask[i, j+1, k] * m_c
        diff_w_y = (nu_n * grad_n - nu_s * grad_s) / dy
        
        # z
        grad_b = (w_c - buffers.w[i, j, k-1]) / dz_m * buffers.mask[i, j, k-1] * m_c
        grad_t = (buffers.w[i, j, k+1] - w_c) / dz_p * buffers.mask[i, j, k+1] * m_c
